...
var horizontalComponent = prime({
	...
	'_getEmberComponents': function(user, renderer, callback) {
		...
		.then(function(hasPermission) {
			if(hasPermission) {
				return promises.all([
					filesystem.readFileAsync(path.join(self.basePath, 'ember-stuff/components/horizontal-menu-manager-widget.js'), 'utf8')
				]);
			}
	...
	'_getEmberComponentHTMLs': function(user, renderer, callback) {
		...
		.then(function(hasPermission) {
			if(hasPermission) {
				return promises.all([
					filesystem.readFileAsync(path.join(self.basePath, 'ember-stuff/componentHTMLs/horizontal-menu-manager-widget.ejs'), 'utf8')
				]);
			}
