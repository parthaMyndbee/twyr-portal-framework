/*
 * Name			: app/modules/components/menus/components/horizontal/component.js
 * Author		: Vish Desai (vishwakarma_d@hotmail.com)
 * Version		: 0.7.1
 * Copyright	: Copyright (c) 2014 - 2016 Vish Desai (https://www.linkedin.com/in/vishdesai).
 * License		: The MITNFA License (https://spdx.org/licenses/MITNFA.html).
 * Description	: The Twy'r Portal Horizontal Menus Component - design / render horizontal menus
 *
 */
...
var horizontalComponent = prime({
	...
	'start': function(dependencies, callback) {
		...
		horizontalComponent.parent.start.call(self, dependencies, function(err, status) {
			if(err) {
	...
	'_getEmberComponents': function(user, renderer, callback) {
		...
		.then(function(userHorizontalMenus) {
			...
			if(hasAuthorPermission) {
				userHorizontalMenus.push(filesystem.readFileAsync(path.join(self.basePath, 'ember-stuff/components/horizontal-menu-manager-widget.js'), 'utf8'));
			}

			userHorizontalMenus.push(filesystem.readFileAsync(path.join(self.basePath, 'ember-stuff/components/horizontal-menu-item-view-widget.js'), 'utf8'));
	...
	'_getEmberComponentHTMLs': function(user, renderer, callback) {
		...
		.then(function(userHorizontalMenus) {
			var hasAuthorPermission = userHorizontalMenus.pop();
			if(hasAuthorPermission) {
				userHorizontalMenus.push(filesystem.readFileAsync(path.join(self.basePath, 'ember-stuff/componentHTMLs/horizontal-menu-manager-widget.ejs'), 'utf8'));
			}

			userHorizontalMenus.push(filesystem.readFileAsync(path.join(self.basePath, 'ember-stuff/componentHTMLs/horizontal-menu-item-view-widget.ejs'), 'utf8'));
			return promises.all(userHorizontalMenus);
		})
...
exports.component = horizontalComponent;
