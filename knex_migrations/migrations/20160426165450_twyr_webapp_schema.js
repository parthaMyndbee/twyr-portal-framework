...
exports.up = function(knex, Promise) {
	...
	.then(function() {
		return Promise.all([
			...
			knex.schema.raw("CREATE TYPE public.tenant_type AS ENUM ('department','suborganization')")
		]);
	})
	...
	.then(function() {
		return Promise.all([
			...
			.createTableIfNotExists('users', function(userTbl) {
				...
				userTbl.specificType('gender', 'public.gender').defaultTo('other');
			...
			.createTableIfNotExists('tenants', function(tenantTbl) {
				...
				tenantTbl.specificType('type', 'public.tenant_type').notNullable().defaultTo('suborganization');
	...
	// Step 14: Create the triggers on all the tables
	.then(function() {
		return Promise.all([
