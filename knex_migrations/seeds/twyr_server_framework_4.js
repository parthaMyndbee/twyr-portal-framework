...
exports.seed = function(knex, Promise) {
	...
	.then(function(existingComponentId) {
		...
		.then(function(sessionComponentId) {
			...
			return knex("module_templates").insert({ 'module': componentId, 'name': 'home', 'description': 'The default Homepage Management Template', 'media_type': 'all', 'user_type': 'all', 'is_default': true }).returning('id');
		})
		.then(function() {
		...
		.then(function(permId) {
			...
			return knex("module_menus").insert({ 'parent': null, 'module': componentId, 'permission': publicPermId, 'ember_route': 'home', 'icon_class': 'fa fa-home', 'display_name': 'Default Home', 'description': 'The default Home Menu that ships with the Portal', 'tooltip': 'Default Home', 'is_default_home': true });
		});
	});
};
